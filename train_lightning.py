...
class GeneratorDataset(Dataset):
    def __init__(self, texts, model: TTS, speaker_ids, speed=1.0, batch_size=1):
        self.texts = texts
        self.model = model
    ...
    def __getitem__(self, idx):
        ...
        for i in range(self.batch_size):
            ...
            self.model.model.dec_training = []
            self.model.model.dec_training.append({"text": text})
            ...
            # load batch of data to train on
            point = self.model.model.dec_training.pop(0)
            # x_in = point["x_in"]
            # g_in = point["g_in"]
            # o_out = point["o_out"]
            data.append((point["x_in"], point["g_in"], point["o_out"]))

        return data


# define the LightningModule
class LitGeneratorWrapper(L.LightningModule):
    def __init__(
        self,
        generator_params,
        learning_rate: float,
        ...
        self.distilled_generator = Generator(**generator_params)

    def training_step(self, batch, batch_idx):
        batch_loss = 0
        for x_in, g_in, o_out in batch:
            x_in = x_in.squeeze(0)
            g_in = g_in.squeeze(0)
            o_out = o_out.squeeze(0)
            output = self.distilled_generator(x_in, g_in)
            loss = F.mse_loss(output, o_out)
            batch_loss += loss.item()
    ...
    def configure_optimizers(self):
        return torch.optim.Adam(
            self.distilled_generator.parameters(), lr=self.learning_rate
        )
...
# train and eval dataset and data loaders
train_dataset = GeneratorDataset(
    texts, model, speaker_ids, speed=speed, batch_size=batch_size
)
train_data_loader = DataLoader(train_dataset, batch_size=1, shuffle=True)

eval_dataset = GeneratorDataset(
    evaluation_texts, model, speaker_ids, speed=speed, batch_size=batch_size
)
eval_data_loader = DataLoader(eval_dataset, batch_size=1, shuffle=False)

model_wrapper = LitGeneratorWrapper(
    generator_params=generator_params,
    learning_rate=lr,
)
# wandb_logger = WandbLogger(project="MeloTTS decoder-generator distillation")
