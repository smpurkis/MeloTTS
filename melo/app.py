...
with gr.Blocks() as demo:
    ...
    with gr.Group():
        speaker = gr.Dropdown(
            speaker_ids.keys(), interactive=True, value="EN-US", label="Speaker"
        )
        language = gr.Radio(
            ["EN", "ES", "FR", "ZH", "JP", "KR"], label="Language", value="EN"
        )
        speed = gr.Slider(
            label="Speed",
            minimum=0.1,
...
def main(share, host, port):
    demo.queue(api_open=False).launch(
        show_api=False, share=share, server_name=host, server_port=port
    )
